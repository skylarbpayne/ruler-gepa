...
class BradleyTerryAggregator:
    ...
    def _update_pair(self, winner: str, loser: str) -> None:
        ...
        # Bradley-Terry in log-odds space uses a standard logistic link.
        score_diff = self.scores[winner] - self.scores[loser]
        p_win = 1 / (1 + exp(-score_diff))
...
def create_aggregator(method: str, **kwargs: Any):
    """Factory function for aggregators."""
    if method == "bradley-terry":
        return BradleyTerryAggregator(
            learning_rate=kwargs.get("learning_rate", 0.1),
            initial_score=kwargs.get("initial_score", 0.0),
        )
    if method == "elo":
        return EloAggregator(
            k_factor=kwargs.get("k_factor", 32.0),
            initial_rating=kwargs.get("initial_rating", 1500.0),
        )
    if method == "copeland":
